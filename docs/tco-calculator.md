...
- **Cost providers** (Owning at Large Hyperscaler Volume / Rent - 3 Year Commit) have different $/GPU/hr rates per GPU
- **Token types** (Total/Input/Output) have different throughput denominators

|                                  | Total   | Input    | Output        |
| -------------------------------- | ------- | -------- | ------------- |
| **Hyperscaler (costh)**          | `costh` | `costhi` | `costhOutput` |
| **Rent - 3 Year Commit (costr)** | `costr` | `costri` | `costrOutput` |
...
- **Cost provider names match `/inference`.** The selector reuses `COST_TIER_LABELS[...].option`
  (Owning at Large Hyperscaler Volume, Rent - 3 Year Commit) plus Custom $/GPU/hr.
