...
| `dataset`                       | `slug`, `convId`, `search`, `sort`, `limit`, `offset`, `expanded`, `turn`, `raw`, `inner`, `sa`                                                                                                                                                                                                                                             |
| `evaluation`                    | `unofficialrun`, `benchmark`, `date`, `format`, `gpus`, `model`, `precisions`                                                                                                                                                                                                                                                               |
| `evaluation-samples`            | `evalResultId`, `runId`, `task`, `model`, `framework`, `hardware`, `precision`, `specMethod`, `disagg`, `concurrency`, `filter`, `offset`, `limit`, `docId`, `search`                                                                                                                                                                       |
...
| `inference`                     | `currentConfig`, `baselineConfig`, `allPoints`, `best`, `date`, `dates`, `deployment`, `end`, `format`, `frameworks`, `gpus`, `metric`, `model`, `optimal`, `percentile`, `power`, `precisions`, `priceSource`, `runId`, `sequence`, `spec`, `start`, `tcoBasis`, `unofficialrun`, `userCosts`, `userPowers`, `vendors`, `xmetric`, `xmode` |
...
See [the control-level parity audit](frontend-api-skills-audit.md) for baseline
gaps, newly added drilldowns, transient controls, exclusions and verification
limits. `control-coverage.test.ts` also checks every persisted share-state key
against an API parameter or an explicit rendering/derived explanation.
...
The catalog, phase/source, dataset flamegraph and evaluation sample projections
now have additional published views; raw readers remain available.
