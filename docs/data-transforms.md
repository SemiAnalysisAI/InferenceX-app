...
- `tokenRevenuePerGpuHour` — gross revenue at a normalized $1/M total-token sale price: `tputPerGpu × 3,600 ÷ 1,000,000` ($/GPU/hr). The explicit unit price keeps the axis comparable across interactivity points. A single equal input/output sale price scales the curve linearly; separate prices require the Fleet Lifecycle model.
