...
- Infrastructure purchasing-power fields divide tokens per hour by the same hourly costs. Output-only (`outputTokensPerDollarH`/`N`/`R`) and input-only (`inputTokensPerDollarH`/`N`/`R`) remain separate USD Y-axis metrics. Total, output, and input infrastructure purchasing power also remain available in CNY through the `tokensPerRmb*` fields. The canonical total USD `tokensPerDollar` field is different: it is the reciprocal of the selected blended token sale price and is computed from the cache-aware revenue path above. Legacy `i_metric=y_tokensPerDollarH|N|R` links resolve to that canonical API-price metric. The dashboard defaults to token revenue per GPU hour so fixed-price scenarios still compare hardware throughput.
