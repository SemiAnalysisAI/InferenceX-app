...
- Purchasing-power fields — tokens-per-hour divided by GPU hourly cost: `costh` / `costn` / `costr` for hyperscaler / neocloud / 3-year-rental pricing respectively. The legacy `cost*` identifiers remain stable for existing shared URLs, but their chart values are tokens purchasable per $1. Three token variants exist: combined (`costh`/`costn`/`costr`), output-only (`costhOutput`/`costnOutput`/`costrOutput`), input-only (`costhi`/`costni`/`costri`).
...
**Limits** — both charts include `y_latency_limit: 60` (filter x-axis outliers beyond 60s when TTFT is on x). Purchasing-power metrics have no fixed Y-axis clamp.
