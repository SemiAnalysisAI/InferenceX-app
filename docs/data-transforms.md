...
- Token-cost fields — tokens-per-hour divided by GPU hourly cost: `costh` / `costn` / `costr` for hyperscaler / neocloud / 3-year-rental pricing respectively. The legacy `cost*` identifiers remain stable for existing shared URLs, and the stored chart value is tokens purchasable per $1. The `costDisplayMode` presentation state defaults to that stored value or derives cost per million tokens as its exact reciprocal (`1_000_000 / tokensPerDollar`); it does not independently interpolate the reciprocal. Three token variants exist: combined (`costh`/`costn`/`costr`), output-only (`costhOutput`/`costnOutput`/`costrOutput`), input-only (`costhi`/`costni`/`costri`).
...
5. **Build renderable graphs** — `stableChartDefinitions` is computed in a separate `useMemo` that depends only on metric/axis selections (not on data). This decouples Y-axis changes from data changes so D3 Effect 3 (metric repositioning) does not fire alongside Effect 2 (data bind). Within this memo, the x-axis field is resolved per chart type, roofline directions are flipped when the x-axis polarity reverses (e.g. interactivity → TTFT), and Y-axis labels are looked up. Cost display mode also replaces the label and vertically flips the Pareto direction for the reciprocal cost-per-million view. The final `graphs` memo applies GPU filtering, optional user-cost/user-power overrides, remaps each point's `x`/`y` from the selected metric's `{ y, roof }` object, and applies the reciprocal only after retrieving the stored tokens-per-dollar value.
...
**Limits** — both charts include `y_latency_limit: 60` (filter x-axis outliers beyond 60s when TTFT is on x). Tokens-per-dollar purchasing power has no fixed Y-axis clamp. Selecting cost per million tokens restores the original `$5/M tok` display boundary; clipped points remain available to the dashed continuation layer.
