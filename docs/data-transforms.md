...
- `tokenRevenuePerGwYear` and `tokenProfitPerGwYearH/N/R` share the `tokenRevenuePerGpuHour` pricing path (`TOKEN_SALE_PRICING_METRIC_KEYS` in `token-revenue.ts`). GPU-hours per all-in utility GW-year are `(1,000,000 / power) × 8760`, where `power` is the all-in kW per GPU from `HW_REGISTRY`. Revenue per GW-year is `$/GPU/hr revenue × GPU-hours`; profit per GW-year is `($/GPU/hr revenue − costh|costn|costr) × GPU-hours` and can be negative, so the linear y-domain in `ScatterGraph`, `GPUGraph`, and `TrendChart` extends below zero only when data does. `applyTokenRevenuePricing` deletes and recomputes all five keys together; hardware without a `power` or tier cost figure gets no value. Historical Trends interpolates priced $/GPU/hr revenue once and maps it through `tokenSalePricingMetricFromRevenuePerGpuHour`. Axis labels and titles for every sale-priced metric name the active price source via `tokenSalePricingLabels`.
