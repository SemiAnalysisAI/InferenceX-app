...
describe('Qwen3.5 measured P75 and P90 backfills', () => {
  it('ties every exact selector and value to its raw audit evidence', () => {
    expect(
      QWEN35_P90_POWER_BACKFILLS.map((backfill) => backfill.productionBenchmarkId).toSorted(),
    ).toEqual(
      evidence.records.map((record: { row_id: string }) => Number(record.row_id)).toSorted(),
    );
