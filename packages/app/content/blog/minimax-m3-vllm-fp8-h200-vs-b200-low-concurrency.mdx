...
<Figure
  ...
  alt="MiniMax M3 vLLM FP8 throughput per GPU vs interactivity, ISL/OSL 1024/1024, B200 vs H200. H200 sits above B200 across the high-interactivity (low-concurrency) band; B200's curve falls off steeply above ~60 tok/s/user where its only recipes are the EP8 throughput configs."
  caption="MiniMax M3 (428B, MoE) on vLLM FP8 at ISL 1024 / OSL 1024, non-MTP, measured 2026-06-13. Both SKUs span TP=4 and TP=8 recipes with EP=1/4/8. The B200 frontier collapses at high interactivity because its low-batch points run the DeepGEMM MoE path; H200 runs Marlin."
/>
