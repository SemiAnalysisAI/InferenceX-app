...
export function GET(request: NextRequest): Promise<Response> {
  return runViewsRoute('calculator', async () => {
    validateViewParams(request.nextUrl.searchParams, VIEW_QUERY_PARAMS['calculator']);
    ...
    const { grouped, groupMeta, hwConfigMap } = buildGpuGroups<GroupMeta>(
      [...rows, ...overlayRows],
      {
        sequence,
        precisions,
        ...
        tcoBasis,
        classify: (hwKey, row) => {
          const prefix = overlayRows.includes(row) ? `${row.run_url}|` : '';
          return multiPrecision
            ? {
                key: `${prefix}${hwKey}__${row.precision}`,
                meta: { hwKey, precision: row.precision },
              }
            : { key: `${prefix}${hwKey}`, meta: { hwKey } };
        },
      },
    );
      ...
      const fleet =
        mw === undefined
          ...
          : computeFleetStatsForResult(
              ...
              meta.hwKey,
              mw,
              { costProvider, costType, tcoBasis },
            );

      hardware.push({
        source: groupKey.includes('|') ? groupKey.split('|')[0] : 'official',
        hwKey: meta.hwKey,
        resultKey: groupKey,
