...
const { mockGetAllEvalResults, mockGetDb, mockUnofficial } = vi.hoisted(() => ({
...
beforeEach(() => {
  ...
  mockUnofficial.mockImplementation(() => Response.json({ evaluations: [] }));
});

describe('GET /api/v1/views/evaluation', () => {
  it('accepts public run IDs without changing the official date domain and keeps sources separate', async () => {
      ...
      Response.json({
        evaluations: [
          evalRow({
            id: 99,
            date: '2026-04-01',
            run_url: 'https://github.com/SemiAnalysisAI/InferenceX/actions/runs/456',
          }),
        ],
      }),
    );
    const response = await GET(
      request(
        `/api/v1/views/evaluation?model=${MODEL}&benchmark=gsm8k&unofficialrun=456&gpus=h200`,
      ),
    );
    expect(response.status).toBe(200);
    const body = await response.json();
    expect(body.params.date).toBe('2026-03-05');
    expect(body.rows).toHaveLength(2);
    expect(body.rows.map((row: { source: string }) => row.source)).toEqual([
      'official',
      'https://github.com/SemiAnalysisAI/InferenceX/actions/runs/456',
    ]);
    expect(mockUnofficial.mock.calls[0][0].nextUrl.searchParams.get('runId')).toBe('456');
  });
  ...
  it('returns a CSV representation with format=csv', async () => {
    ...
    expect(lines[0]).toBe('source,hwKey,label,score,stderr,n,precision,framework,date');
