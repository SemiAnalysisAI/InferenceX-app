...
export function GET(request: NextRequest) {
  return runViewsRoute('reliability', async () => {
    validateViewParams(request.nextUrl.searchParams, VIEW_QUERY_PARAMS['reliability']);
    ...
    const asOf = parseDateParam(search.get('asOf'), 'asOf');
    ...
    return cachedJson({
      view: 'reliability',
      apiVersion: 'v1',
      params: { range, format, gpus, asOf: asOf ?? null },
      range,
      hardware,
