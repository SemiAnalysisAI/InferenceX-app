...
describe('artifact → local DB → real Blob cache → point API recovery', () => {
  ...
  it('logs failed cache writes, serves repaired data, and retries on the next request', async () => {
    await ingestGpuMetricsArtifact(sql, {
      workflowRunId: 1,
      artifact: artifact(),
      benchmarkResultIds: [10],
    ...
    await request(10);
    await db.exec(
      `UPDATE benchmark_results SET power_audit = '{"source":"repaired"}' WHERE id = 10`,
    );
    blob.rejectWrites(true);
    const warning = vi.spyOn(console, 'warn').mockImplementation(() => {});
    try {
      const fresh = await request(10);
      expect(fresh.response.status).toBe(200);
      expect(fresh.body.series[0].powerAudits).toEqual([{ source: 'repaired' }]);
      expect(blob.counts.failedWrites).toBeGreaterThan(0);
      expect(warning).toHaveBeenCalledWith(expect.stringContaining('serving uncached result'));
      blob.rejectWrites(false);
      const writes = blob.counts.writes;
      const recovered = await request(10);
      expect(recovered.body).toEqual(fresh.body);
      expect(blob.counts.writes).toBe(writes + 1);
      await request(10);
      expect(blob.counts.writes).toBe(writes + 1);
    } finally {
      warning.mockRestore();
  ...
  it('does not serve a cached success when the revision DB read fails', async () => {
    await ingestGpuMetricsArtifact(sql, {
      workflowRunId: 1,
      artifact: artifact(),
      benchmarkResultIds: [10],
    ...
    await request(10);
    connection.getDb.mockImplementationOnce(() => {
      throw new Error('local DB unavailable');
    });
    const log = vi.spyOn(console, 'error').mockImplementation(() => {});
    try {
      const failed = await request(10);
