...
import { buildSpecDecodeMetadata, renderSpecDecodePage } from '../page';

/**
 * `/compare-spec-decode/<slug>/<scenario>` — the same comparison with its workload pinned by
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildSpecDecodeMetadata(slug, { scenarioSegment: assertScenario(scenario) });
}

export default async function SpecDecodeScenarioPage({ params, searchParams }: Props) {
  const { slug, scenario } = await params;
  return renderSpecDecodePage(slug, await searchParams, {
