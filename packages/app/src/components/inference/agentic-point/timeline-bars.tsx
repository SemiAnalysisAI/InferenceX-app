...
export interface TimelineBarsProps {
  ...
  /** Absolute index of rows[0] within the full virtualized row list. */
...
export const TimelineBars = memo(
  ({
    ...
    return (
      <>
        <defs>
          <clipPath id="timeline-body-clip">
            <rect
              x={0}
              y={HEADER_HEIGHT}
              width={CHART_WIDTH}
              height={Math.max(0, viewportHeight - HEADER_HEIGHT)}
            />
          </clipPath>
        </defs>
        ...
        <g clipPath="url(#timeline-body-clip)">
          {/* Row separators */}
          {rows.map((row, idx) => (
            <line
              key={`sep-${row.key}`}
              x1={0}
              y1={HEADER_HEIGHT + (firstRowIndex + idx) * ROW_SPAN - scrollTop}
              x2={CHART_WIDTH}
              y2={HEADER_HEIGHT + (firstRowIndex + idx) * ROW_SPAN - scrollTop}
              stroke="currentColor"
              opacity={0.04}
            />
          ))}
          ...
          {rows.map((row, rowIdx) => {
            const yTop =
              HEADER_HEIGHT + (firstRowIndex + rowIdx) * (ROW_HEIGHT + ROW_GAP) - scrollTop + 2;
