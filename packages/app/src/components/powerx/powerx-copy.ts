...
export const POWERX_STRINGS = {
  ...
  zh: {
    model: `模型草案（${SYSTEM_POWER_MODEL_REVISION}）：GPU 机箱交流功率加 PUE ${SYSTEM_POWER_ASSUMPTIONS.pue}；CPU 利用率为 ${SYSTEM_POWER_ASSUMPTIONS.u_cpu * 100}%，DRAM 利用率为 ${SYSTEM_POWER_ASSUMPTIONS.u_ram * 100}%。不足整机的配置先按实测每 GPU 功率推算整机，再分摊所用 GPU 的份额。不包含独立 CPU 路由节点。不支持的硬件或场景显示不可用。这不是墙插功率实测值。`,
    provisioned:
