...
describe('serializePerfRulers / parsePerfRulers', () => {
  ...
  it('survives URLSearchParams encoding, which the share link applies', () => {
    const state = complete(EMPTY_PERF_RULER_STATE, OFFICIAL_A, OVERLAY, 33);
    const search = new URLSearchParams({ i_rulers: serializePerfRulers(state) });
    const decoded = new URLSearchParams(search.toString()).get('i_rulers');
    expect(parsePerfRulers(decoded).rulers).toEqual([
      { id: 1, curveA: OFFICIAL_A, curveB: OVERLAY, isoX: 33 },
    ]);
  });

  it('rounds the iso-x to four significant digits and never emits the draft', () => {
    let state = complete(EMPTY_PERF_RULER_STATE, OFFICIAL_A, OFFICIAL_B, 123.456789);
    state = nextPerfRulerState(state, { curve: OFFICIAL_B, isoX: 9 });
    expect(state.draft).not.toBeNull();
    expect(serializePerfRulers(state)).toBe(`123.5|${OFFICIAL_A}|${OFFICIAL_B}`);
    expect(serializePerfRulers(complete(EMPTY_PERF_RULER_STATE, 'a', 'b', 0.00012346))).toBe(
      '0.0001235|a|b',
    );
    expect(serializePerfRulers(complete(EMPTY_PERF_RULER_STATE, 'a', 'b', 98765))).toBe(
      '98770|a|b',
    );
  });

  it('serializes the empty state to the empty string so the param strips as default', () => {
    expect(serializePerfRulers(EMPTY_PERF_RULER_STATE)).toBe('');
    const drafted = nextPerfRulerState(EMPTY_PERF_RULER_STATE, { curve: 'a', isoX: 1 });
    expect(serializePerfRulers(drafted)).toBe('');
  });

  it('returns the shared empty state for missing or entirely malformed input', () => {
    expect(parsePerfRulers('')).toBe(EMPTY_PERF_RULER_STATE);
    expect(parsePerfRulers(null)).toBe(EMPTY_PERF_RULER_STATE);
    expect(parsePerfRulers(undefined)).toBe(EMPTY_PERF_RULER_STATE);
    expect(parsePerfRulers('garbage')).toBe(EMPTY_PERF_RULER_STATE);
    expect(parsePerfRulers(';;|')).toBe(EMPTY_PERF_RULER_STATE);
  });

  it('drops malformed entries and keeps the valid ones', () => {
    const encoded = [
      `NaN|${OFFICIAL_A}|${OFFICIAL_B}`, // non-numeric iso-x
      `|${OFFICIAL_A}|${OFFICIAL_B}`, // missing iso-x
      `12|${OFFICIAL_A}`, // missing curve B
      `12||${OFFICIAL_B}`, // empty curve A
      `12|${OFFICIAL_A}|${OFFICIAL_A}`, // a curve cannot be measured against itself
      `12|${OFFICIAL_A}|${OFFICIAL_B}|extra`, // too many fields
      `Infinity|${OFFICIAL_A}|${OFFICIAL_B}`,
      `7.5|${OFFICIAL_A}|${OFFICIAL_B}`,
    ].join(';');
    expect(parsePerfRulers(encoded).rulers).toEqual([
      { id: 1, curveA: OFFICIAL_A, curveB: OFFICIAL_B, isoX: 7.5 },
    ]);
  });

  it('only accepts roofline identity classes as curve ids', () => {
    const encoded = [
      // The shared marker classes sit on EVERY roofline path: a ruler between
      // them would land on whichever two paths come first in the DOM.
      `10|roofline-path|overlay-roofline-path`,
      `10|${OFFICIAL_A}|roofline-path`,
      // Other nodes inside the zoom group are not curves.
      `10|perf-ruler-hit|${OFFICIAL_A}`,
      `10|dot-group|zoom-group`,
      `10|${OFFICIAL_A}|roofline-`, // identity class needs a series
      `10|${OFFICIAL_A}|roofline-b200 fp8`, // no whitespace in a class token
      `10|${OFFICIAL_A}|${OVERLAY}`,
    ].join(';');
    expect(parsePerfRulers(encoded).rulers).toEqual([
      { id: 1, curveA: OFFICIAL_A, curveB: OVERLAY, isoX: 10 },
    ]);
  });

  it('caps at MAX_PERF_RULERS keeping the newest entries, like the click reducer', () => {
    const encoded = Array.from(
      { length: MAX_PERF_RULERS + 3 },
      (_, index) => `${index + 1}|roofline-a${index}|roofline-b${index}`,
    ).join(';');
    const parsed = parsePerfRulers(encoded);
    expect(parsed.rulers).toHaveLength(MAX_PERF_RULERS);
    expect(parsed.rulers[0]).toEqual({
      id: 1,
      curveA: 'roofline-a3',
      curveB: 'roofline-b3',
      isoX: 4,
    });
    expect(parsed.rulers.at(-1)).toEqual({
      id: MAX_PERF_RULERS,
      curveA: `roofline-a${MAX_PERF_RULERS + 2}`,
      curveB: `roofline-b${MAX_PERF_RULERS + 2}`,
      isoX: MAX_PERF_RULERS + 3,
    });
    expect(parsed.nextId).toBe(MAX_PERF_RULERS + 1);
  });

  it('hands out ids a later click never reuses', () => {
    const restored = parsePerfRulers(`10|roofline-a|roofline-b;20|roofline-c|roofline-d`);
    const placed = complete(restored, 'e', 'f', 30);
    expect(placed.rulers.map((ruler) => ruler.id)).toEqual([1, 2, 3]);
  });
});
