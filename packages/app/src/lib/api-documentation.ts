...
export const apiOperations: readonly ApiOperation[] = [
  ...
  },
  {
    ...
    description: text(
      'Returns the stored plain-text server log for one benchmark result ID. Add offset or limit to request a bounded chunk; chunked reads are recommended because some logs are hundreds of megabytes.',
      '返回某个基准结果 ID 已存储的纯文本服务器日志。可添加 offset 或 limit 读取有界分块；部分日志达到数百 MB，建议使用分块读取。',
    ),
    audience: 'public',
    stability: 'beta',
    parameters: [
    ...
    curlUrl: `${API_BASE_URL}/api/v1/server-log?id=421&offset=0&limit=65536`,
  },
  {
