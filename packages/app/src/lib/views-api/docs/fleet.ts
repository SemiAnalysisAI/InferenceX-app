...
export const operations: ApiOperation[] = [
  {
    ...
    parameters: [
      ...
      parameter(
        ...
        false,
        'CSV list',
        'Comma-separated precisions. Omit to auto-select the densest precision in the data. Model and scenario pairs with a fixed default open on it instead (GLM-5.2/GLM-5.3 agentic-traces: fp4 and fp8).',
        '以逗号分隔的精度列表。省略时自动选择数据中曲线最密的精度。部分模型与场景组合改用固定默认集合（GLM-5.2/GLM-5.3 智能体编码：fp4 与 fp8）。',
        { type: 'string' },
        'fp4',
      ),
      parameter(
