...
export const operations: ApiOperation[] = [
  {
    ...
    parameters: [
      ...
      parameter(
        ...
        false,
        'CSV list',
        'Comma-separated precisions. Omit to auto-select the densest official precision and include precisions present in unofficial-run overlays.',
        '以逗号分隔的精度列表。省略时选择官方数据中曲线最多的精度，并纳入 unofficial-run 叠加数据中的精度。',
        { type: 'string' },
        'fp4,fp8',
      ),
      parameter(
      ...
      parameter(
        ...
        false,
        'enum',
        'Cost basis: costh (owning) or costr (renting), with tcoBasis selecting internal or external assumptions.',
        '成本口径：costh（自有）或 costr（租赁）；tcoBasis 选择 internal 或 external 假设。',
        { type: 'string', enum: ['costh', 'costr'], default: 'costh' },
        'costh',
      ),
      parameter(
        'costType',
