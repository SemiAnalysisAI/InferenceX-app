...
export const operations: ApiOperation[] = [
  {
    ...
    parameters: [
      ...
      parameter(
        ...
        false,
        'CSV list',
        'Comma-separated precisions. Omit to auto-select the densest official precision and include precisions present in unofficial-run overlays. Model and scenario pairs with a fixed default open on it instead (GLM-5.2/GLM-5.3 agentic-traces: fp4 and fp8).',
        '以逗号分隔的精度列表。省略时选择官方数据中曲线最多的精度，并纳入 unofficial-run 叠加数据中的精度。部分模型与场景组合改用固定默认集合（GLM-5.2/GLM-5.3 智能体编码：fp4 与 fp8）。',
        { type: 'string' },
        'fp4,fp8',
      ),
      parameter(
