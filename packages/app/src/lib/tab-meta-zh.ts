...
export const TAB_META_ZH: Record<ZhTabKey, { title: string; description: string }> = {
  ...
  historical: {
    title: '历史推理性能趋势',
    description:
      '跟踪 AI 推理性能随时间的变化。历史基准测试数据展示各 Chip 与服务商在延迟、吞吐量和成本上的改进。',
  },
  calculator: {
    title: '吞吐量与 TCO 计算器',
    description:
      '计算 AI 推理吞吐量与总拥有成本（TCO）。跨硬件配置对比 LLM 推理服务的 Chip 成本效益。',
  },
  reliability: {
    title: '服务商可靠性指标',
    description: 'AI 推理服务商可靠性与可用性跟踪。对比各 Chip 云服务商的错误率与可用性。',
  },
  'gpu-specs': {
    title: 'Chip 规格与对比',
    description:
      '面向 AI 推理的详细 Chip 规格。对比 NVIDIA、AMD 与 Intel Chip 的显存带宽、FLOPS、互连与拓扑。',
  },
  'gpu-metrics': {
    title: 'Chip 功耗与能效指标',
    description: 'AI 推理负载下的 Chip 功耗与能效指标。跨硬件对比每瓦 token 数。',
  },
  collectivex: {
    title: 'CollectiveX 通信基准测试',
    description:
      '跨 NVIDIA 与 AMD Chip 平台对比专家并行（EP）通信性能，包括混合专家（MoE）分发（dispatch）、合并（combine）延迟与逻辑载荷速率。',
  },
  submissions: {
    title: '基准测试提交记录',
    description:
      '提交到 InferenceX 的全部基准测试配置。查看各 Chip 厂商的提交历史、活动趋势与数据点数量。',
  },
  'ai-chart': {
    title: 'AI 驱动的图表生成',
    description: '使用自然语言提示生成自定义推理基准测试图表。借助 AI 对比 Chip、成本与性能。',
  },
  'current-inferencex-image': {
    title: 'InferenceX 当前镜像',
    description:
      '各模型、Chip SKU 和配置的当前 InferenceX Docker 镜像标签。对比已部署镜像与最新 vLLM 和 SGLang 发布版本，标记过期标签。',
  },
  feedback: {
    title: '用户反馈',
...
export const TAB_INTRO_ZH: Record<ZhTabKey, string> = {
  ...
  historical:
    '本页面展示历史趋势图表：跟踪各 Chip、框架与模型的推理性能随时间的演进，量化软件栈优化带来的收益。',
  calculator:
    '本页面提供吞吐量与总拥有成本（TCO）计算器：基于真实基准测试数据，估算不同 Chip 配置下 LLM 推理服务的每百万 token 成本与性价比。',
  reliability:
    '本页面展示基准测试基础设施的可靠性指标：各 Chip 集群与服务商的运行成功率、错误率与可用性。',
  'gpu-specs':
    '本页面提供 Chip 规格对比：NVIDIA、AMD 等厂商加速器的显存容量、显存带宽、FLOPS、互连拓扑与功耗规格。',
  'gpu-metrics':
    '本页面展示 Chip 功耗与能效指标（PowerX）：推理负载下的实测功耗、每瓦 token 数与每兆瓦 token 产出。',
  collectivex:
  ...
  submissions:
    '本页面列出提交到 InferenceX 的全部基准测试配置：按 Chip 厂商查看提交历史、活动趋势与数据点数量。',
  'ai-chart':
    '本页面提供 AI 驱动的图表生成工具：用自然语言描述您想查看的图表，系统会根据 InferenceX 基准测试数据自动生成可视化结果。',
  'current-inferencex-image':
    '本页面展示 InferenceX 当前使用的 Docker 镜像标签：按模型、Chip SKU 和配置列出已部署版本，并与上游 vLLM、SGLang 最新发布版本对比，方便排查过期镜像。',
  feedback:
