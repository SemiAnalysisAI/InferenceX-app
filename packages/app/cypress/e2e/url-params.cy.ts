...
describe('URL Parameter Persistence', () => {
  ...
  describe('Perf ruler share state', () => {
    const chart = '#chart-0 svg';

    // The second click completes a ruler only when the two curves share an
    // x range; real fixture curves do not all overlap, so walk the hit
    // strokes until one pairs with the first.
    const completeRuler = (index: number) => {
      cy.get(`${chart} .perf-ruler-hit`)
        .its('length')
        .then((count) => {
          expect(index, 'a curve overlapping the first one').to.be.lessThan(count);
          cy.get(`${chart} .perf-ruler-hit`).eq(index).click({ force: true });
          cy.get(chart).then(($svg) => {
            if ($svg.find('.perf-ruler').length === 0) completeRuler(index + 1);
          });
        });
    };

    it('round-trips a placed ruler through the share link and restores it on load', () => {
      visitWithDismissedModal('/inference?g_model=DeepSeek-R1-0528&i_best=0');
      cy.get(`${chart} .roofline-path`).should('have.length.greaterThan', 1);
      ...
      cy.get(`${chart} .perf-ruler-hit`).should('have.length.greaterThan', 1);
      cy.get(`${chart} .perf-ruler-hit`).eq(0).click({ force: true });
      completeRuler(1);
      cy.get(`${chart} .perf-ruler`).should('have.length', 1);

      cy.get(`${chart} .perf-ruler .pr-text-ratio`)
        .invoke('text')
        .then((ratio) => {
          expect(ratio).to.match(/^\d+\.\d{2}x$/u);
          // The address bar stays clean after load, so the share link is
          // where the placed ruler must show up.
          cy.get('[data-testid="share-button"]').first().click();
          cy.get('[data-testid="share-url-input"]')
            .invoke('val')
            .then((value) => {
              const shareUrl = new URL(String(value));
              const rulers = shareUrl.searchParams.get('i_rulers');
              expect(rulers, 'i_rulers in the share link').to.match(
                /^[\d.e+-]+\|roofline-[\w%.-]+\|roofline-[\w%.-]+$/u,
              );

              // Opening the link reproduces the ruler without a click and
              // switches the tool on so it is visible.
              visitWithDismissedModal(`${shareUrl.pathname}${shareUrl.search}`);
              cy.get(`${chart} .perf-ruler`).should('have.length', 1);
              // The link stores the iso-x to four significant digits, so the
              // restored ratio may differ from the placed one in the last
              // displayed decimal.
              cy.get(`${chart} .perf-ruler .pr-text-ratio`)
                .invoke('text')
                .then((restoredRatio) => {
                  expect(Number.parseFloat(restoredRatio)).to.be.closeTo(
              ...
              cy.get('[data-testid="share-url-input"]')
                .invoke('val')
                .then((restoredValue) => {
                  expect(new URL(String(restoredValue)).searchParams.get('i_rulers')).to.equal(
                    rulers,
                  );
                });
              cy.get('body').type('{esc}');

              // Clearing the rulers drops the param from the next share link.
              cy.get('[data-testid="scatter-clear-perf-rulers"]').click();
              cy.get(`${chart} .perf-ruler`).should('not.exist');
              cy.get('[data-testid="share-button"]').first().click();
              cy.get('[data-testid="share-url-input"]')
                .invoke('val')
                .should('not.include', 'i_rulers');
            });
        });
    });

    it('clears a restored ruler when the x-axis mode changes the rendered chart', () => {
      // Build the link through the UI so it references curves the fixture
      // really renders, then reopen it and swap the x axis: chart-0 now
      // draws the e2e graph in seconds, so the ruler placed in tok/s/user
      // must go, on screen and in the next share link.
      visitWithDismissedModal('/inference?g_model=DeepSeek-R1-0528&i_best=0');
      cy.get(`${chart} .roofline-path`).should('have.length.greaterThan', 1);
      expandLegendAdvanced();
      cy.get('#scatter-perf-ruler').click({ force: true });
      ...
      cy.get('[data-testid="share-url-input"]')
        ...
        .then((value) => {
          const shareUrl = new URL(String(value));
          expect(shareUrl.searchParams.get('i_rulers')).to.include('|roofline-');
          visitWithDismissedModal(`${shareUrl.pathname}${shareUrl.search}`);
          cy.get(`${chart} .perf-ruler`).should('have.length', 1);
          ...
          cy.get('[data-testid="share-url-input"]')
            .invoke('val')
            .should('include', 'i_xmode=e2e')
            .and('not.include', 'i_rulers');
        });
    });

    it('ignores a malformed i_rulers value and leaves the tool off', () => {
      visitWithDismissedModal('/inference?g_model=DeepSeek-R1-0528&i_rulers=not-a-ruler');
      cy.get(`${chart} .roofline-path`).should('have.length.greaterThan', 1);
      cy.get(`${chart} .perf-ruler`).should('not.exist');
      expandLegendAdvanced();
      cy.get('#scatter-perf-ruler').should('have.attr', 'aria-checked', 'false');
