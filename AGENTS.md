...
6. **What stays English**: brand/product names, hardware SKUs, model/framework/precision names, units (tok/s/user, GB/s, $/M tok), code identifiers and flags — per the translation quality bar — plus the context-aware technical loanwords documented in `docs/chinese-copy.md` (including `warmup`, `seed`, and `offload` in first-party ML infrastructure copy), and DB-stored _content_ (benchmark rows, dataset conversation text, run logs), which is data, not UI.
